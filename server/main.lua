...
--自動繳費的code
RegisterNetEvent('esx_billing:pay2')
AddEventHandler('esx_billing:pay2', function()
	local xPlayer = ESX.GetPlayerFromId(source)
	local identity= ESX.GetPlayerFromId(source).identifier
--連結sql(資料庫)
	MySQL.Async.fetchAll('SELECT * FROM billing WHERE identifier = @identifier', {
		['@identifier'] = identity
	}, function(result1)
		for i=1, #result1, 1 do
			...
			['@id'] = result1[i].id
				...
				local xTarget = ESX.GetPlayerFromIdentifier(sender)
				if targetType == 'player' then
					MySQL.Async.execute('DELETE from billing WHERE id = @id', {   --繳罰單
						['@id'] = result1[i].id
					}, function(rowsChanged)
						xPlayer.removeMoney(amount*Config.paymorebill)--扣錢
						xTarget.addMoney(amount*Config.paymorebill) --加錢
						TriggerClientEvent('esx:showNotification', xPlayer.source, _U('paid_invoice', ESX.Math.GroupDigits(amount*Config.paymorebill)))
						TriggerClientEvent('esx:showNotification', xTarget.source, _U('received_payment', ESX.Math.GroupDigits(amount*Config.paymorebill)))
					end)
				else
					...
					TriggerEvent('esx_addonaccount:getSharedAccount', target, function(account)
						MySQL.Async.execute('DELETE from billing WHERE id = @id', { --繳罰單
							...
							xPlayer.removeMoney(amount*Config.paymorebill)--扣錢
							account.addMoney(amount*Config.paymorebill) --加錢
							TriggerClientEvent('esx:showNotification', xPlayer.source, _U('paid_invoice', ESX.Math.GroupDigits(amount*Config.paymorebill)))
							if xTarget ~= nil then
								TriggerClientEvent('esx:showNotification', xTarget.source, _U('received_payment', ESX.Math.GroupDigits(amount*Config.paymorebill)))
							end
